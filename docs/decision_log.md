...
actually do" signals (keeper/defender decisions had none), plus a first-time-visitor path
into the site instead of dropping straight into a data page.

| # | Decision | Choice | Rationale |
|---|---|---|---|
| 37 | Goalkeeper/defender decision signal | **New `mart_team_defensive_outlook`**: a 0–100 score blending clean-sheet rate so far this season (60%) with upcoming fixture ease (40%), surfaced as a "🧤 Clean sheet picks" section on Transfer Decisions (best keeper + best defender per top-ranked team) | `mart_transfer_decision`'s own signals barely speak to defense — its underlying-threat component is an attacking metric (npxG+xA) that sits neutral for every goalkeeper and only rewards a defender's own attacking returns, never their clean-sheet record. Rather than reopening `transfer_score`'s just-finalized weights (#33/#34) to fold this in, it ships as a clearly separate, purpose-built signal — lower risk, easier to defend on its own terms. Clean-sheet rate excludes teams with `played = 0` from its percentile (same "excluded, not scored 0" treatment as #33's underlying_pctl) rather than let a missing value rank artificially well; a missing signal coalesces to neutral 0.5, same convention as `mart_transfer_decision`. |
...
| 51 | New "Leaderboard" page | **List a manager's private classic mini-leagues from `entry/{id}/`'s `leagues.classic`, filtered to `league_type == "x"`, then show `leagues-classic/{id}/standings/` for whichever is picked, flagging the manager's own row** | Same shape as #50: they have this, we didn't, and the data (`leagues.classic`) was already sitting unused in the exact payload My Team already fetches. Verified live against several real team IDs first (not just id=1, which turned out to have zero private leagues — id=2 has five, confirming both the empty-state and populated-state paths against real data before writing UI code). System leagues (`league_type == "s"`, e.g. "Overall") are excluded on purpose — real data, but not what a manager means by "mini-league," and often far too large to render as a table. |

## A real visual identity, not just stock Streamlit (2026-07-16)

Direct user feedback: every card looked identical, the site had no color beyond
chart series, and it didn't read as a polished product. Root cause: no `.streamlit/config.toml`
existed at all, so the whole app ran on Streamlit's plain default theme — `app/ui.py`'s
validated dataviz-skill palette (decision log's chart-color entries) was only ever wired
into chart marks, never into page chrome, cards, or status/position meaning.

| # | Decision | Choice | Rationale |
|---|---|---|---|
| 52 | App-wide theme | **New `.streamlit/config.toml`**: dark base, violet primary (`#9085e9`, the palette's own categorical slot 7), the palette's dark page-plane/chart-surface as background/secondary-background, status colors mapped onto Streamlit's basic palette slots | Same *already-validated* reference palette the chart code already used (decision log's earlier dataviz-skill entries) — applied one layer up, at the theme level, instead of inventing new colors. Free wins for every built-in widget (sliders, progress bars, dataframes, alert boxes) with zero per-page code. |
| 53 | Status color coding | **Fixed mapping** `transfer_in`→good green, `hold`→neutral blue, `monitor`→warning yellow, `drop`→critical red (the palette's own reserved status steps, `hold` borrowing the neutral categorical blue since it isn't good or bad) — rendered as a card's left border via a CSS rule keyed to `st.container`'s `key` param (`st-key-status-{status}-...`), never the sole channel (icon + label, already established, stays) | Verified empirically first that Streamlit's `key` param becomes CSS class `st-key-{sanitized key}` on the bordered wrapper div (read the compiled frontend bundle, then confirmed live via a real screenshot) before rolling it out — not assumed from memory. Applied to every `player_mini_card` call site (movers/budget/differentials/clean-sheet/captain/top-pick) and My Team's weak-link cards; the *same* channel means the *same* thing everywhere on the site, deliberately, rather than switching meaning per section. |
| 54 | Position color coding | **Fixed mapping** GKP→violet, DEF→aqua, MID→orange, FWD→magenta (the four categorical slots not already spoken for by status) as a small colored badge next to a player's name, and as a translucent row-wash on tabular views (Live's top performers) via the same `df.style.apply()` technique `league_table.py`'s `zone_tint` already established | A second, orthogonal channel from status (border vs. badge/wash) so the two never collide in meaning. GKP intentionally reuses the brand-violet hue — goalkeepers already wear a visually distinct kit in real football, so it reads as intentional rather than a clash with the primary accent appearing elsewhere. |
| 55 | Home's four highlight cards | **Distinct top-border accent per card** (violet/blue/aqua/orange) via the same `key`-based CSS technique, one fixed list (`HIGHLIGHT_COLORS`), not status or position — these are four unrelated destinations, not four entities of the same type | This was the most visible instance of "every card looks the same," being the very first thing a visitor sees. Not fixed in the first pass — caught on a self-review screenshot after building the rest of the site, which is why the fix landed as its own follow-up commit rather than in the same pass as #53/54. |
| 56 | Verification method | **Real headless-browser screenshots** (Selenium + the system's Chrome, with an explicit `WebDriverWait` for rendered content), not just `AppTest` | `AppTest` proves the script runs without exceptions but renders no pixels — insufficient for a *visual* request. Plain `chrome --headless --screenshot` was tried first and silently failed (captures at the `load` event, before Streamlit's WebSocket-delivered content ever paints) — root-caused via a zoomed single-element screenshot before concluding the technique itself, not the app, was broken. |

Caught mid-build: `verdict_tint`/`you_tint`-style row functions read a column (`recommendation`) that
had been sliced out of the display dataframe before styling — `st.dataframe(df[display_cols].style.apply(...))`
throws `KeyError` inside the Styler, not at the call site, so it only surfaces when the table
actually renders with real data. Fixed by keeping the needed column in the frame and hiding
it from view with `column_order` instead of slicing it out beforehand. Both `AppTest`-with-real-data
